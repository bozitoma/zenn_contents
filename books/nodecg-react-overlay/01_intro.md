...
これまで、[NodeCG](https://www.nodecg.dev/ja/)というフレームワークを使って、**配信用のオーバーレイを制御するアプリ**をいくつか開発してきました。
本書では、自分用に作成した**NodeCGアプリの開発用テンプレート（Vite + React + TypeScript）を公開**し、ハンズオン形式でNodeCGを体系的に学べるように解説していきます。

NodeCGについて知りたい人、自作で配信オーバーレイのアプリを作りたい人は、ぜひ参考にしてください。
...
## 1. NodeCGとは？
[NodeCG](https://www.nodecg.dev/ja/)は、Node.jsとブラウザをベースに、**ライブ配信用のオーバーレイ（映像に重ねる動的なグラフィック）を制御するアプリケーションの開発に特化したフレームワーク**です。
